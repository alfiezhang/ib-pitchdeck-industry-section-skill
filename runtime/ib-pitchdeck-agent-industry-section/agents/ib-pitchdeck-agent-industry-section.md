...
description: Pre-mandate investment banking pitchbook industry-section agent. Use when a user provides a target brief, PDF, PPT, URL, industry report, template, or project lead and asks for an industry section or PPT.
displayName:
  en: "IB Pitchdeck Industry Section"
  zh: "投行Pitch行业章节"
profession:
  en: "Investment Banking Pitchbook Industry Section Specialist"
  zh: "投行Pitchbook行业章节专家"
maxTurns: 120
---
...
Set up the runtime from the plugin package directory, then work from **your
project directory** (the folder where you started this session):

```bash
PLUGIN_DIR="/path/to/ib-pitchdeck-agent-industry-section"
...
**current working directory** (`$PWD`), never inside the plugin package
directory. `<work_root>` means `$PWD`, not the plugin package path.
...
This plugin is organized as a role-based workflow. This main agent is the
Orchestrator; it routes work to the smallest relevant role skill.

| Role | Skill file | Owns |
|---|---|---|
| Orchestrator | `agents/ib-pitchdeck-agent-industry-section.md` | run state, stage routing, package-of-record discipline |
| Material Intake | `skills/material-intake/SKILL.md` | user text/files/URLs/reports, source classification, normalized input card |
| Knowledge Repository | `skills/knowledge-repository/SKILL.md` | facts, metrics, sources, conflicts, evidence DB, research-pack export |
| Industry Scoping | `skills/industry-scoping/SKILL.md` | broad/core/adjacent/excluded boundary and boundary validation loop |
| Research / External Evidence | `skills/research-external-evidence/SKILL.md` | formal search plan, public search, manual URL/PDF ingestion, source reviews, archive, execution accounting |
| Reasoning | `skills/reasoning/SKILL.md` | supported judgments, hypotheses, research requests, issue analysis, page arguments |
| Generation | `skills/generation/SKILL.md` | deck blueprint, page design, evidence contract, renderer spec |
| Template | `skills/template/SKILL.md` | template profile, style/capacity fit, template QC routing |
| QC | `skills/qc/SKILL.md` | validators, gates, final delivery checks, repair targets |
| Output | `skills/output/SKILL.md` | deterministic PPT rendering and final package |

No legacy subskill path is part of the main runtime workflow. If older
automation references removed compatibility paths, update it to the role model
above instead of recreating compatibility wrappers.
...
3. The role skill for the current stage from the Role Model above.
...
`materials -> input card -> knowledge intake -> industry scoping -> boundary validation -> formal research -> source reviews/archive -> execution accounting -> research_evidence_db -> generated research pack -> reasoning/issue analysis -> page argument/deck blueprint -> template fit -> compiled contract/renderer -> formal pipeline -> final delivery`
...
2. **Material intake**: classify user text/files/URLs/reports, extract explicit
   facts, and create a transcription-only `input_card.json`.
...
8. **Reasoning loop**: resolve hypotheses into supported judgments, public
   research requests, or caveat/diligence question blocks.
9. **Deck blueprint**: design pages, headline/main message/body blocks, visual
...
- If only 10 searches were actually executed out of 40+ planned `FS-xxx` rows,
  ...
  or body claims.
- `artifacts/coverage_accounting.json` is the separate planned/executed/not
  executed ledger. It supports audit and repair routing; it is not research
  evidence and must not be treated as source material in issue analysis.
...
`template_profile.json` must be generated by `scripts/template_analyzer.py` and
include `render_layouts`, `page_type_capability`, `source_area`, and
`density_budget`. Do not repair profile failures by patching the run artifact.
