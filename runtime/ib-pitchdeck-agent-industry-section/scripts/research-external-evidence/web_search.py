...
Usage:
  python scripts/research-external-evidence/web_search.py -q "中国目标行业市场规模"
  python scripts/research-external-evidence/web_search.py -q "示例公司 营收" --provider tavily -n 3
  python scripts/research-external-evidence/web_search.py --queries "目标行业市场规模" "示例公司营收" "目标行业趋势" -o tmp/search_batch.json
  python scripts/research-external-evidence/web_search.py -q "中国目标细分市场发展现状及未来趋势分析" --split-query
  python scripts/research-external-evidence/web_search.py -q "年度报告 营收" --site cninfo.com.cn --site-mode priority
  python scripts/research-external-evidence/web_search.py -q "行业趋势" --source-pack china_official --source-registry configs/source_registry.json
  python scripts/research-external-evidence/web_search.py -q "行业趋势" --use-default-packs --source-registry configs/source_registry.json
...
DEFAULT_SOURCE_REGISTRY = _IB_RUNTIME_ROOT / "configs" / "source_registry.json"
...
# from configs/source_registry.json and research policy, not from project- or
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to configs/source_registry.json for search connector config.",
    )
    parser.add_argument(
        "--searxng-url",
        default="",
    ...
    parser.add_argument(
        "--source-registry",
        default=str(DEFAULT_SOURCE_REGISTRY),
        help="Path to configs/source_registry.json for resolving source packs"
    )
    parser.add_argument(
