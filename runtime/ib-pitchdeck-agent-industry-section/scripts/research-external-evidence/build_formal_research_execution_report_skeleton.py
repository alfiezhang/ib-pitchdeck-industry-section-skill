#!/usr/bin/env python3
"""Build a formal_research_execution_report skeleton from plan/log/archive.
...
maps archived sources to SRC-xxx rows so the LLM only has to confirm status,
...
def _load_archive_reviews(path: Path | None) -> list[dict[str, Any]]:
    if not path or not path.exists():
        return []
    data = load_json_file(path)
    ...
    for item in _as_list(raw):
        ...
        reviews.append(
            _canonical_review(
                {
                    "source_review_id": item.get("source_review_id") or item.get("archive_id") or item.get("source_id"),
                    "url": item.get("url"),
                    "title": item.get("title"),
                    "source_type": item.get("source_type"),
                    "source_access": item.get("source_access"),
                    "locator": item.get("locator"),
                    "excerpt": item.get("reviewed_excerpt") or item.get("excerpt"),
                    "search_attempt_ids": item.get("search_attempt_ids"),
                    "evidence_ids": item.get("evidence_ids"),
                    "evidence_use_tier": item.get("evidence_use_tier") or "candidate",
                    "usable_as_evidence": item.get("usable_as_evidence") if isinstance(item.get("usable_as_evidence"), bool) else False,
                    "review_status": item.get("review_status") or "needs_llm_source_review",
                    "limitations": item.get("limitations") or item.get("archive_unavailable_reason"),
                }
            )
        )
    return reviews
...
def _merge_reviews(*review_lists: list[dict[str, Any]]) -> list[dict[str, Any]]:
    by_key: dict[str, dict[str, Any]] = {}
    for review_list in review_lists:
        for review in review_list:
            key = _text(review.get("source_review_id")) or _norm_url(_text(review.get("url")))
            if not key:
                continue
            by_key[key] = {**by_key.get(key, {}), **review}
    ...
    *,
    ...
    for instruction in _planned_instructions(plan):
        ...
        if usable_reviews:
            ...
            findings_summary = "Auto-built execution skeleton: archived source(s) are linked; confirm the finding and limitations before research-pack promotion."
            handling = "Review linked archived sources inside research_evidence_db and promote only confirmed facts/metrics into Formal Research Extracts, Evidence Ledger, and Metric Reconciliation."
            covered_issue_areas.add(instruction["issue_area"])
            thin_or_unresolved.append(f"{instruction['issue_area']}/{instruction['subissue']}")
        elif attempts:
            status = "insufficient"
            limitations = ["Formal search attempt exists, but archived source support is not yet reviewed as usable evidence."]
            findings_summary = "Formal search was executed and archived, but source review/evidence support is not yet sufficient for promotion."
            handling = "Review archived sources in research_evidence_db, promote confirmed facts/metrics, or keep this as a research gap/backlog."
