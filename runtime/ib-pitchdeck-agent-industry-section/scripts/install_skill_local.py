#!/usr/bin/env python3
"""Install the runtime skill package into a local host skills directory."""
...
from skill_package_common import RUNTIME_ROOT, SKILL_NAME, default_target_root, ensure_inside, iter_package_files, relpath
QC_SYSTEM_VALIDATORS = RUNTIME_ROOT / "scripts" / "qc" / "validators" / "system"
if str(QC_SYSTEM_VALIDATORS) not in sys.path:
    sys.path.insert(0, str(QC_SYSTEM_VALIDATORS))

from validate_skill_package import validate_entries
from skill_package_common import package_entries_from_dir, package_entries_from_zip
...
def _extract_zip_clean(zip_path: Path, target_dir: Path) -> int:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        prefix = ""
        candidates = [name for name in names if name.endswith("SKILL.md")]
        if candidates:
            prefix = sorted(candidates, key=len)[0][: -len("SKILL.md")]
        for name in names:
            ...
            if rel_path.is_absolute() or any(part in {"", ".", ".."} for part in rel_path.parts):
                raise ValueError(f"unsafe path in skill zip: {name}")
...
def install_skill(source: Path, host: str, target_root: Path | None = None) -> dict:
    ...
    if not validation["is_valid"]:
        return {
            "schema_version": "skill_local_install_v1",
            "is_valid": False,
            "installed": False,
            "host": host,
...
def main() -> int:
    ...
    parser.add_argument("--target-root", help="Override host skills root; useful for tests or custom skill sources")
