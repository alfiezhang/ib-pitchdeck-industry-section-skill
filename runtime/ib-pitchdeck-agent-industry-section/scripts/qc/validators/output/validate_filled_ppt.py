...
def collect_active_placeholders(ppt_mapping: dict, control_data: dict, control_file_path: Path) -> list[dict]:
    ...
    for mapping_slide in ppt_mapping.get("slides", []):
        ...
        if variant is None:
            ...
            raise ValueError(
                f"Mapping mismatch in configs/ppt_mapping.json: slide_no={slide_no}, slide_key={slide_key}, "
...
def optional_placeholder_fields(control_data: dict) -> set[tuple[int, str]]:
    ...
    for slide in slides:
        ...
        if slide_no == 6 and slide.get("selected_page_type") == "compare_table_page":
            compare_table = slide.get("compare_table_data")
            if isinstance(compare_table, dict):
                row_count = len([row for row in compare_table.get("rows") or [] if isinstance(row, dict)])
                if row_count >= 3:
                    for idx in range(row_count + 1, 7):
                        optional.add((slide_no, f"table_row_{idx}"))
    return optional


def collect_suspicious_missing_values(replacement_dict: dict, active_placeholders: list[dict], control_data: dict) -> list[dict]:
    suspicious = []
    optional_fields = optional_placeholder_fields(control_data)
    for item in active_placeholders:
        if (
            ...
            continue
        if (item["slide_no"], item["field_name"]) in optional_fields:
            continue
...
def build_report(
    ...
    suspicious_missing_values = collect_suspicious_missing_values(replacement_dict, active_placeholders, control_file)
...
def main():
    ...
    parser.add_argument("--control-file", default="renderer_spec.json",
                        help="Path to slide control file: renderer_spec.json.")
    parser.add_argument("--replacement-dict", default="replacement_dict.json")
    parser.add_argument("--ppt-mapping", default="configs/ppt_mapping.json")
