...
DEFAULT_LAYOUT_BUDGET_PATH = _IB_RUNTIME_ROOT / "configs" / "layout_budget.json"
DEFAULT_DRILLDOWN_ROLE_LIBRARY_PATH = _IB_RUNTIME_ROOT / "configs" / "drilldown_role_library.json"
# ── Helpers ──────────────────────────────────────────────────────
...
def validate_slide_1_2_pair(
    ...
    if not role_2:
        message = (
            ...
            "from configs/drilldown_role_library.json (e.g., market_segmentation, channel_structure, "
        ...
        message = (
            f"slide 2: drilldown_role/page_role '{role_2}' is not in configs/drilldown_role_library.json. "
        ...
        warnings.append(
            ...
            "use one role_id from configs/drilldown_role_library.json."
        )
...
CONTENT_REPAIR_PROFILES: dict[str, dict[str, Any]] = {
    ...
    "WEAK_OR_GENERIC_SOURCE": {
        ...
        "repair_hint": "Replace generic source notes with specific EV IDs and source locators; if the EV/source is missing or not auditable, repair the research evidence DB or source archive first.",
    },
    "EVIDENCE_LINKAGE": {
        "category": "source_traceability",
...
def main() -> None:
    ...
    parser.add_argument(
        "--rules", required=True,
        help="Path to configs/content_quality_rules.json."
    )
    parser.add_argument(
        "--text-fit-rules",
        help="Optional path to configs/text_fit_rules.json. Defaults to sibling file next to --rules when present."
    )
    parser.add_argument(
        "--layout-budget",
        default=str(DEFAULT_LAYOUT_BUDGET_PATH),
        help="Optional path to configs/layout_budget.json."
    )
    parser.add_argument(
        "--output",
