...
REPAIR_PROFILES: dict[str, dict[str, Any]] = {
    ...
    "mechanical_alias_or_enum": {
        "repair_target": "industry_issue_analysis.json",
        "repair_hint": "Run skills/reasoning/scripts/normalize_issue_analysis.py first, then revalidate. This handles common aliases such as sufficient/thin statuses and renamed fields.",
        "repair_fields": ["issue_analyses[].status", "issue_analyses[].analysis_type", "issue_analyses[].evidence_sufficiency"],
        "helper": "skills/reasoning/scripts/normalize_issue_analysis.py",
    },
    "thin_analysis_text": {
        "repair_target": "industry_issue_analysis.json",
...
def build_repair_plan(errors: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "instruction": (
            ...
            "then run state_report.py next before moving downstream."
        ),
        "primary_repair_targets": primary_targets,
        "targets": targets,
        "rerun_steps": [
            "skills/reasoning/scripts/normalize_issue_analysis.py if mechanical_alias_or_enum appears",
