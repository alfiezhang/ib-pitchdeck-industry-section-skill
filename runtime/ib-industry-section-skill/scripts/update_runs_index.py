...
def attempt_summary(attempt_dir: Path) -> dict[str, Any]:
    ...
    debug_only = (attempt_dir / "DEBUG_OUTPUT_ONLY.txt").exists()
...
def write_index(
    ...
    for item in sorted(summaries, key=lambda row: row["name"], reverse=True):
        ...
        if item.get("debug_only"):
            issues.append("debug-only run; not eligible for latest-final pointer")
        lines.append(
