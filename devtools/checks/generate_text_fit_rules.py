...
LIB_DIR = RUNTIME_ROOT / "scripts" / "_lib"
...
def build_rules(template_path: Path, ppt_mapping_path: Path, title_font_size: float) -> dict[str, Any]:
    ...
    return {
        "_description": (
            "Template-derived line-fit rules for renderer spec headline/main_message and "
            "template token fields slide_title/main_takeaway. Main takeaway width is intentionally "
...
def main() -> None:
    ...
    parser.add_argument("--output", default="configs/text_fit_rules.json")
    parser.add_argument(
        "--title-font-size",
        type=float,
        default=28.0,
