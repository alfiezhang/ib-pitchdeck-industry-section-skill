#!/usr/bin/env python3
"""Shared helpers for skill package/install scripts."""
...
SKILL_NAME = "ib-pitchdeck-agent-industry-section"
REPO_ROOT = Path(__file__).resolve().parents[2]
RUNTIME_ROOT = REPO_ROOT / "runtime" / SKILL_NAME
...
def detect_skill_root(entries: dict[str, bytes]) -> str:
    candidates = [name for name in entries if name.endswith("SKILL.md")]
    if not candidates:
        return ""
...
def normalize_entries(entries: dict[str, bytes]) -> dict[str, bytes]:
    root = detect_skill_root(entries)
    if not root:
        return entries
